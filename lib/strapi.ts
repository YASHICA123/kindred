...
export async function fetchSchools() {
  try {
    ...
    if (!res.ok) {
      console.warn(`Strapi unavailable (${res.status}), using mock data`)
    ...
    if (data.length === 0) {
      console.warn('Strapi returned no schools, using mock data')
      return MOCK_SCHOOLS.map(normalizeSchool)
    }
    
    return data.map(normalizeSchool)
  } catch (error) {
    console.warn('Error fetching from Strapi, using mock data:', error)
    return MOCK_SCHOOLS.map(normalizeSchool)
  }
}
